...
/**
 * This class tracks state which changes over the course of a layout run. Generally speaking, if possible, state
 * information should be stored in the box tree and not here. It also provides pass-though calls to many methods in
 * {@link SharedContext}.
 */
public class LayoutContext implements CssContext {
	...
	LayoutContext(LayoutContext context) {
		_sharedContext = context._sharedContext;

		_rootLayer = context._rootLayer;

		_firstLines = context._firstLines;
		_firstLetters = context._firstLetters;
		_currentMarkerData = context._currentMarkerData;

		_bfcs = context._bfcs;
		_layers = context._layers;

		_fontContext = context._fontContext;

		_contentFunctionFactory = context._contentFunctionFactory;

		_extraSpaceTop = context._extraSpaceTop;
		_extraSpaceBottom = context._extraSpaceBottom;

		_counterContextMap = context._counterContextMap;

		_pendingPageName = context._pendingPageName;
		_pageName = context._pageName;

		_noPageBreak = context._noPageBreak;

		_rootDocumentLayer = context._rootDocumentLayer;
		_page = context._page;

		_mayCheckKeepTogether = context._mayCheckKeepTogether;

		_breakAtLineContext = context._breakAtLineContext;
	}
	...
	public class CounterContext {
		...
		/**
		 * This is different because it needs to work even when the counter- properties cascade and it should also
		 * logically be redefined on each level (think list-items within list-items)
		 */
		private CounterContext _parent;

		/**
		 * A CounterContext should really be reflected in the element hierarchy, but CalculatedStyles reflect the
		 * ancestor hierarchy just as well and also handles pseudo-elements seamlessly.
		 * 
		 * @param style
		 */
		CounterContext(CalculatedStyle style) {
			...
			if (resets != null)
				for (Iterator i = resets.iterator(); i.hasNext();) {
			...
			if (increments != null)
				for (Iterator i = increments.iterator(); i.hasNext();) {
					CounterData cd = (CounterData) i.next();
