...
/**
 * A block box as defined in the CSS spec. It also provides a base class for other kinds of block content (for example
 * table rows or cells).
 */
public class BlockBox extends Box implements InlinePaintable {
	...
	public String dump(LayoutContext c, String indent, int which) {
		...
		result.append(" styleMargin=[" + styleMargin.top() + ", " + styleMargin.right() + ", " + styleMargin.bottom() + ", "
				+ styleMargin.right() + "] ");
	...
	public void positionAbsolute(CssContext cssCtx, int direction) {
		...
		if ((direction & POSITION_HORIZONTALLY) != 0) {
			...
			if (!style.isIdent(CSSName.LEFT, IdentValue.AUTO)) {
				...
				setX(boundingBox.width
						- (int) style.getFloatPropertyProportionalWidth(CSSName.RIGHT, getContainingBlock().getContentWidth(), cssCtx)
						- getWidth());
			}
			setX(getX() + boundingBox.x);
			System.out.println("I moved stuff to the right");
		}
		...
		if ((direction & POSITION_VERTICALLY) != 0) {
			...
			if (!style.isIdent(CSSName.TOP, IdentValue.AUTO)) {
				...
				setY(boundingBox.height - (int) style.getFloatPropertyProportionalWidth(CSSName.BOTTOM, cbContentHeight, cssCtx)
						- getHeight());
			}
	...
	protected void calcDimensions(LayoutContext c, int cssWidth) {
		if (!isDimensionsCalculated()) {
			...
			if (cssWidth != -1
					&& !isAnonymous()
					&& (getStyle().isIdent(CSSName.MARGIN_LEFT, IdentValue.AUTO) || getStyle().isIdent(CSSName.MARGIN_RIGHT,
							IdentValue.AUTO)) && getStyle().isNeedAutoMarginResolution()) {
			...
			if (!isAnonymous() || (isFromCaptionedTable() && isFloated())) {
				...
				if (re != null) {
					...
					setReplacedElement(re);
						...
						&& (style.isInlineBlock() || style.isFloated() || style.isAbsolute() || style.isFixed())) {
	...
	protected void calcLayoutHeight(LayoutContext c, BorderPropertySet border, RectPropertySet margin, RectPropertySet padding) {
		setHeight(getHeight()
				+ ((int) margin.top() + (int) border.top() + (int) padding.top() + (int) padding.bottom() + (int) border.bottom() + (int) margin
						.bottom()));
	...
	private void collapseMargins(LayoutContext c) {
		if (!isTopMarginCalculated() || !isBottomMarginCalculated()) {
			...
			if (!isTopMarginCalculated() && !isBottomMarginCalculated() && isVerticalMarginsAdjoin(c)) {
				MarginCollapseResult collapsedMargin = _pendingCollapseCalculation != null ? _pendingCollapseCalculation
						: new MarginCollapseResult();
	...
	private boolean isVerticalMarginsAdjoin(LayoutContext c) {
		...
		boolean bordersOrPadding = (int) borderWidth.top() != 0 || (int) borderWidth.bottom() != 0 || (int) padding.top() != 0
				|| (int) padding.bottom() != 0;
		...
		if (!isAnonymous()) {
			if (!getStyle().isAutoWidth()) {
				if (shrinkingToFit && !getStyle().isAbsoluteWidth()) {
					...
					int result = (int) getStyle().getFloatPropertyProportionalWidth(CSSName.WIDTH, getContainingBlock().getContentWidth(),
							c);
	...
	protected int getCSSFitToWidth(CssContext c) {
		if (!isAnonymous()) {
			if (!getStyle().isIdent(CSSName.FS_FIT_IMAGES_TO_WIDTH, IdentValue.AUTO)) {
				int result = (int) getStyle().getFloatPropertyProportionalWidth(CSSName.FS_FIT_IMAGES_TO_WIDTH,
		...
		if (!isAnonymous()) {
			if (!isAutoHeight()) {
				if (getStyle().hasAbsoluteUnit(CSSName.HEIGHT)) {
					...
					return (int) getStyle().getFloatPropertyProportionalHeight(CSSName.HEIGHT,
							((BlockBox) getContainingBlock()).getCSSHeight(c), c);
				}
			}
		}
	...
	public void calcMinMaxWidth(LayoutContext c) {
		if (!isMinMaxCalculated()) {
			...
			if (isReplaced() || (width != -1 && !isFixedWidthAdvisoryOnly())) {
				_minWidth = _maxWidth = (int) margin.left() + (int) border.left() + (int) padding.left() + width + (int) margin.right()
						+ (int) border.right() + (int) padding.right();
			} else {
				int cw = -1;
				if (width != -1) {
				...
				_minWidth = _maxWidth = (int) margin.left() + (int) border.left() + (int) padding.left() + (int) margin.right()
						+ (int) border.right() + (int) padding.right();
				...
				if (getChildrenContentType() == CONTENT_BLOCK || getChildrenContentType() == CONTENT_INLINE) {
					switch (getChildrenContentType()) {
					case CONTENT_BLOCK:
	...
	private ReplacedElement fitReplacedElement(LayoutContext c, ReplacedElement re) {
		...
		if (maxImageWidth > -1 && re.getIntrinsicWidth() > maxImageWidth) {
			...
			re = c.getReplacedElementFactory().createReplacedElement(c, this, c.getUac(), maxImageWidth,
					(int) Math.rint(scale * re.getIntrinsicHeight()));
		}
		return re;
	}
	...
	private void calcMinMaxCSSMinMaxWidth(LayoutContext c, RectPropertySet margin, BorderPropertySet border, RectPropertySet padding) {
		int cssMinWidth = getCSSMinWidth(c);
		if (cssMinWidth > 0) {
			cssMinWidth += (int) margin.left() + (int) border.left() + (int) padding.left() + (int) margin.right() + (int) border.right()
					+ (int) padding.right();
		...
		if (!getStyle().isMaxWidthNone()) {
			int cssMaxWidth = getCSSMaxWidth(c);
			cssMaxWidth += (int) margin.left() + (int) border.left() + (int) padding.left() + (int) margin.right() + (int) border.right()
					+ (int) padding.right();
	...
	public boolean checkPageContext(LayoutContext c) {
		if (!getStyle().isIdent(CSSName.PAGE, IdentValue.AUTO)) {
			...
			if ((!pageName.equals(c.getPageName())) && isInDocumentFlow() && isContainsInlineContent(c)) {
	...
	protected void propagateExtraSpace(LayoutContext c, ContentLimitContainer parentContainer, ContentLimitContainer currentContainer,
 ...
 * $Id$
 * 
 * $Log$ Revision 1.102 2010/01/13 00:42:11 peterbrant Calculate clearance before possibly establishing new BFC. It
 * needs to be calculated relative to the existing BFC and not the new one.
 * 
 * Revision 1.101 2010/01/12 14:33:27 peterbrant Ignore auto margins when calculating table min/max width. Also, when
 * deciding whether or not to proceed with the auto margin calculation for a table, make sure we compare consistently
 * with how the table min width is actually set.
 * 
 * Revision 1.100 2009/11/08 23:52:48 peterbrant Treat percentage widths as auto when calculating min/max widths
 * 
 * Revision 1.99 2008/12/14 19:20:05 peterbrant Skip running blocks when calculating min/max widths
 * 
 * Revision 1.98 2008/12/14 13:53:31 peterbrant Implement -fs-keep-with-inline: keep property that instructs FS to try
 * to avoid breaking a box so that only borders and padding appear on a page
 * 
 * Revision 1.97 2008/09/06 18:21:50 peterbrant Need to account for list-marker-position: inside when calculating inline
 * min/max widths
 * 
 * Revision 1.96 2008/07/27 00:21:47 peterbrant Implement CMYK color support for PDF output, starting with patch from
 * Mykola Gurov / Banish java.awt.Color from FS core layout classes
 * 
 * Revision 1.95 2008/07/14 11:12:35 peterbrant Fix two bugs when -fs-table-paginate is paginate. Block boxes in cells
 ...
 * 
 * Revision 1.94 2008/06/18 17:44:48 peterbrant Fix a pair of NPEs when absolutely positioned content is positioned off
 * the page
 * 
 * Revision 1.93 2008/05/24 16:36:22 peterbrant If our minimum width is greater than the calculated CSS width, don't try
 * to allocate any margin space to auto margins.
 * 
 * Revision 1.92 2007/10/15 22:33:44 peterbrant Only try to satisfy widows and orphans if page breaking is allowed
 * 
 * Revision 1.91 2007/09/19 22:50:43 peterbrant Fix nested percentage height calculations (per report and test case from
 * Simon Buettner)
 * 
 * Revision 1.90 2007/08/29 22:18:17 peterbrant Experiment with text justification
 * 
 * Revision 1.89 2007/08/28 22:31:26 peterbrant Implement widows and orphans properties
 * 
 * Revision 1.88 2007/08/27 19:28:50 peterbrant Enable extra page clearance calculation
 * 
 * Revision 1.87 2007/08/24 18:36:08 peterbrant Further progress on AcroForm support
 * 
 * Revision 1.86 2007/08/23 20:52:31 peterbrant Begin work on AcroForm support
 * 
 * Revision 1.85 2007/08/19 22:22:49 peterbrant Merge R8pbrant changes to HEAD
 * 
 * Revision 1.84.2.13 2007/08/19 21:55:20 peterbrant Try to keep block boxes out of no man's land when paginating a
 * table (commented out)
 * 
 * Revision 1.84.2.12 2007/08/19 20:14:16 peterbrant Support nested paginated tables
 * 
 * Revision 1.84.2.11 2007/08/17 23:53:32 peterbrant Get rid of layer hack for overflow: hidden
 * 
 * Revision 1.84.2.10 2007/08/17 19:11:30 peterbrant When paginating a table, move table past page break if header would
 * overlap the page break
 * 
 * Revision 1.84.2.9 2007/08/17 17:11:00 peterbrant Try to avoid awkward row splits when paginating a table by making
 * sure there is at least one line box on the same page as the start of the row
 * 
 * Revision 1.84.2.8 2007/08/13 22:41:13 peterbrant First pass at exporting the render tree as text
 * 
 * Revision 1.84.2.7 2007/08/09 20:18:15 peterbrant Bug fixes to improved pagination support
 * 
 * Revision 1.84.2.6 2007/08/09 17:03:15 peterbrant Finish running block implementation
 * 
 * Revision 1.84.2.5 2007/08/08 21:44:09 peterbrant Implement more flexible page numbering
 * 
 * Revision 1.84.2.4 2007/08/08 18:28:25 peterbrant Further progress on CSS3 paged media
 * 
 * Revision 1.84.2.3 2007/08/07 17:06:32 peterbrant Implement named pages / Implement page-break-before/after:
 * left/right / Experiment with efficient selection
 * 
 * Revision 1.84.2.2 2007/07/11 22:48:30 peterbrant Further progress on running headers and footers
 * 
 * Revision 1.84.2.1 2007/07/09 22:18:03 peterbrant Begin work on running headers and footers and named pages
 * 
 * Revision 1.84 2007/06/16 22:51:24 tobega We now support counters!
 * 
 * Revision 1.83 2007/06/13 15:04:55 peterbrant Remove obsolete counter related code
 * 
 * Revision 1.82 2007/06/07 16:56:29 peterbrant When vertically aligning table cell content, call layout again on cells
 ...
 * 
 * Revision 1.81 2007/06/05 19:29:53 peterbrant More progress on counter support
 * 
 * Revision 1.80 2007/06/02 06:56:44 peterbrant Table page clearance should be taken into account when checking whether
 * or not the top margin should be reset when a box is moved to a new page
 * 
 * Revision 1.79 2007/04/25 18:09:41 peterbrant Always reset block box margin if it is the first thing on a page
 * 
 * Revision 1.78 2007/04/23 21:13:16 peterbrant Calculate table cell height as if it included borders and padding
 * (matches FF and Opera behavior)
 * 
 * Revision 1.77 2007/04/16 01:10:05 peterbrant Vertical margin and padding with percentage values may be incorrect if
 * box participated in a shrink-to-fit calculation. Fix margin calculation.
 * 
 * Revision 1.76 2007/04/15 00:34:40 peterbrant Allow inline-block / inline-table content to be relatively positioned
 * 
 * Revision 1.75 2007/04/12 12:29:10 peterbrant Properly handle floated tables with captions
 * 
 * Revision 1.74 2007/03/17 22:55:51 peterbrant Remove distinction between box IDs and named anchors
 * 
 * Revision 1.73 2007/03/12 21:11:20 peterbrant Documentation update
 * 
 * Revision 1.72 2007/03/08 01:41:50 peterbrant Don't calculate clearance for floated boxes (clear on floated boxes is
 * handled directly by FloatManager) / Make sure we have a root layer before checking whether we have a fixed background
 * or not
 * 
 * Revision 1.71 2007/03/07 20:34:51 peterbrant Set a provisional content width on table cells when calulating min/max
 * width to make sure percentage values in children resolve to something other than zero / Make sure style changes
 * correctly account for text-transform
 * 
 * Revision 1.70 2007/03/02 00:45:15 peterbrant Calculate baseline correctly for inline-block and inline-table elements
 * 
 * Revision 1.69 2007/02/24 01:57:30 peterbrant toString() changes
 * 
 * Revision 1.68 2007/02/23 15:50:37 peterbrant Fix incorrect absolute box positioning with print medium
 * 
 * Revision 1.67 2007/02/22 18:21:19 peterbrant Add support for overflow: visible/hidden
 * 
 * Revision 1.66 2007/02/22 16:10:54 peterbrant Remove unused API
 * 
 * Revision 1.65 2007/02/22 15:30:42 peterbrant Internal links should be able to target block boxes too (plus other
 * minor cleanup)
 * 
 * Revision 1.64 2007/02/21 23:49:41 peterbrant Can't calculate clearance until margins have been collapsed / Clearance
 * must be calculated relative to the box's border edge, not margin edge
 * 
 * Revision 1.63 2007/02/21 21:47:26 peterbrant Implement (limited) support for collapsing through blocks with adjoining
 * top and bottom margins / <body> should collapse with children
 * 
 * Revision 1.62 2007/02/21 17:17:04 peterbrant Calculate position of next child and block height independently. They
 * may not move in lockstep in the face of negative vertical margins.
 * 
 * Revision 1.61 2007/02/20 23:46:06 peterbrant Include pseudo element in toString()
 * 
 * Revision 1.60 2007/02/11 23:10:59 peterbrant Make sure bounds information is calculated for fixed layers
 * 
 * Revision 1.59 2007/02/07 16:33:25 peterbrant Initial commit of rewritten table support and associated refactorings
 * 
 * Revision 1.58 2006/10/04 23:52:57 peterbrant Implement support for margin: auto (centering blocks in their containing
 * block)
 * 
 * Revision 1.57 2006/10/04 21:35:49 peterbrant Allow dimensions of absolutely positioned content to be specified with
 * all four corners, not just one of left/right, top/bottom
 * 
 * Revision 1.56 2006/10/04 19:49:08 peterbrant Improve calculation of available width when calculating shrink-to-fit
 * width
 * 
 * Revision 1.55 2006/09/08 15:41:58 peterbrant Calculate containing block width accurately when collapsing margins /
 ...
 * 
 * Revision 1.54 2006/09/07 16:24:50 peterbrant Need to calculate (preliminary) box dimensions when collapsing margins
 * 
 * Revision 1.53 2006/09/06 22:42:30 peterbrant Implement min/max-height (non-replaced content only)
 * 
 * Revision 1.52 2006/09/06 22:21:43 peterbrant Fixes to shrink-to-fit implementation / Implement min/max-width
 * (non-replaced content) only
 * 
 * Revision 1.51 2006/09/05 23:03:44 peterbrant Initial draft of shrink-to-fit support
 * 
 * Revision 1.50 2006/09/01 23:49:38 peterbrant Implement basic margin collapsing / Various refactorings in preparation
 * for shrink-to-fit / Add hack to treat auto margins as zero
 * 
 * Revision 1.49 2006/08/30 18:25:41 peterbrant Further refactoring / Bug fix for problem reported by Mike Curtis
 * 
 * Revision 1.48 2006/08/29 17:29:12 peterbrant Make Style object a thing of the past
 * 
 * Revision 1.47 2006/08/27 01:16:20 peterbrant decimal-leading-zero patch from Thomas Palmer
 * 
 * Revision 1.46 2006/08/27 01:15:00 peterbrant Revert makeTextMarker() change to commit with proper attribution
 * 
 * Revision 1.45 2006/08/27 00:36:47 peterbrant Initial commit of (initial) R7 work
 * 
 * Revision 1.44 2006/03/01 00:45:03 peterbrant Provide LayoutContext when calling detach() and friends
 * 
 * Revision 1.43 2006/02/22 02:20:19 peterbrant Links and hover work again
 * 
 * Revision 1.42 2006/02/21 20:55:45 peterbrant Handle image marker failover and list-style-type: none
 * 
 * Revision 1.41 2006/02/21 20:41:15 peterbrant Default to decimal for text list markers
 * 
 * Revision 1.40 2006/02/09 19:12:25 peterbrant Fix bad interaction between page-break-inside: avoid and top:
 * auto/bottom: auto for absolute blocks
 * 
 * Revision 1.39 2006/02/02 02:47:35 peterbrant Support non-AWT images
 * 
 * Revision 1.38 2006/02/01 01:30:13 peterbrant Initial commit of PDF work
 * 
 * Revision 1.37 2006/01/27 01:15:35 peterbrant Start on better support for different output devices
 * 
 * Revision 1.36 2006/01/10 19:56:01 peterbrant Fix inappropriate box resizing when width: auto
 * 
 * Revision 1.35 2006/01/03 23:52:40 peterbrant Remove unhelpful comment
 * 
 * Revision 1.34 2006/01/03 17:04:50 peterbrant Many pagination bug fixes / Add ability to position absolute boxes in
 * margin area
 * 
 * Revision 1.33 2006/01/03 02:12:21 peterbrant Various pagination fixes / Fix fixed positioning
 * 
 * Revision 1.32 2006/01/01 02:38:18 peterbrant Merge more pagination work / Various minor cleanups
 * 
 * Revision 1.31 2005/12/30 01:32:39 peterbrant First merge of parts of pagination work
 * 
 * Revision 1.30 2005/12/21 02:36:29 peterbrant - Calculate absolute positions incrementally (prep work for pagination)
 * - Light cleanup - Fix bug where floats nested in floats could cause the outer float to be positioned in the wrong
 * place
 * 
 * Revision 1.29 2005/12/17 02:24:14 peterbrant Remove last pieces of old (now non-working) clip region checking / Push
 * down handful of fields from Box to BlockBox
 * 
 * Revision 1.28 2005/12/15 20:04:48 peterbrant Implement visibility: hidden
 * 
 * Revision 1.27 2005/12/13 20:46:06 peterbrant Improve list support (implement list-style-position: inside, marker
 * "sticks" to first line box even if there are other block boxes in between, plus other minor fixes) / Experimental
 * support for optionally extending text decorations to box edge vs line edge
 * 
 * Revision 1.26 2005/12/13 02:41:33 peterbrant Initial implementation of vertical-align: top/bottom (not done yet) /
 * Minor cleanup and optimization
 * 
 * Revision 1.25 2005/12/09 21:41:20 peterbrant Finish support for relative inline layers
 * 
 * Revision 1.24 2005/12/09 01:24:56 peterbrant Initial commit of relative inline layers
 * 
 * Revision 1.23 2005/12/05 00:13:53 peterbrant Improve list-item support (marker positioning is now correct) / Start
 * support for relative inline layers
 * 
 * Revision 1.22 2005/11/25 22:38:39 peterbrant Clean imports
 * 
 * Revision 1.21 2005/11/25 16:57:19 peterbrant Initial commit of inline content refactoring
 * 
 * Revision 1.20 2005/11/12 21:55:27 tobega Inline enhancements: block box text decorations, correct line-height when it
 * is a number, better first-letter handling
 * 
 * Revision 1.19 2005/11/08 22:53:46 tobega added getLineHeight method to CalculatedStyle and hacked in some list-item
 * support
 * 
 * Revision 1.18 2005/11/08 20:03:57 peterbrant Further progress on painting order / improved positioning implementation
 * 
 * Revision 1.17 2005/11/05 18:45:06 peterbrant General cleanup / Remove obsolete code
 * 
 * Revision 1.16 2005/10/27 00:09:02 tobega Sorted out Context into RenderingContext and LayoutContext
 * 
 * Revision 1.15 2005/10/21 13:17:15 pdoubleya Rename some methods in RectPropertySet, cleanup.
 * 
 * Revision 1.14 2005/10/21 12:01:20 pdoubleya Added cachable rect property for margin, cleanup minor in styling.
 * 
 * Revision 1.13 2005/10/21 05:52:10 tobega A little more experimenting with flattened render tree
 * 
 * Revision 1.12 2005/10/18 20:57:04 tobega Patch from Peter Brant
 * 
 * Revision 1.11 2005/10/16 23:57:16 tobega Starting experiment with flat representation of render tree
 * 
 * Revision 1.10 2005/10/06 03:20:22 tobega Prettier incremental rendering. Ran into more trouble than expected and some
 * creepy crawlies and a few pages don't look right (forms.xhtml, splash.xhtml)
 * 
 * Revision 1.9 2005/10/02 21:29:59 tobega Fixed a lot of concurrency (and other) issues from incremental rendering.
 * Also some house-cleaning.
 * 
 * Revision 1.8 2005/09/26 22:40:20 tobega Applied patch from Peter Brant concerning margin collapsing
 * 
 * Revision 1.7 2005/01/29 20:24:23 pdoubleya Clean/reformat code. Removed commented blocks, checked copyright.
 * 
 * Revision 1.6 2004/12/16 15:53:10 joshy fixes for absolute layout
 * 
 * Issue number: Obtained from: Submitted by: Reviewed by:
 * 
 * Revision 1.5 2004/12/11 23:36:49 tobega Progressing on cleaning up layout and boxes. Still broken, won't even compile
 * at the moment. Working hard to fix it, though.
 * 
 * Revision 1.4 2004/12/01 01:57:02 joshy more updates for float support.
 * 
 * Issue number: Obtained from: Submitted by: Reviewed by:
 * 
 * Revision 1.3 2004/11/18 16:45:12 joshy improved the float code a bit. now floats are automatically forced to be
 * blocks
 * 
 * 
 * Issue number: Obtained from: Submitted by: Reviewed by:
 * 
 * Revision 1.2 2004/10/23 13:50:26 pdoubleya Re-formatted using JavaStyle tool. Cleaned imports to resolve wildcards
